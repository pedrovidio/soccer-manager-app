...
export default function GroupHomeScreen() {
  ...
  const activeMembers  = members.filter((m) => !m.hasDebt && !m.isInjured);
  const blockedMembers = members.filter((m) => m.hasDebt || m.isInjured);
      ...
      >
        ...
        <View style={s.section}>
          ...
          {membersExpanded && (
            <>
              {activeMembers.map((m) => (
                <MemberRow key={m.id} member={m} isAdmin={isAdmin} groupId={groupId!} onRefresh={refetch} />
              ))}
...
function MemberRow({
  member, isAdmin, groupId, onRefresh,
}: {
  member: GroupMember;
  isAdmin: boolean;
  groupId: string;
  ...
  function handleOptions() {
    ...
    Alert.alert(member.name, 'O que deseja fazer?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: member.isInjured ? 'Marcar como ativo' : 'Marcar como lesionado',
      ...
      },
      {
        ...
        style: 'destructive',
        onPress: () =>
          Alert.alert('Remover membro', `Remover ${member.name} do grupo?`, [
            { text: 'Cancelar', style: 'cancel' },
            {
              text: 'Remover',
              style: 'destructive',
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={isAdmin && !member.isAdmin ? 0.7 : 1}
    >
